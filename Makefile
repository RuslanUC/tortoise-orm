...
test_postgres_asyncpg:
	python -V | grep PyPy || $(py_warn) TORTOISE_TEST_DB="asyncpg://postgres:$(TORTOISE_POSTGRES_PASS)@$(TORTOISE_POSTGRES_HOST):5432/test_\{\}" pytest $(pytest_opts) --cov-report=

test_postgres_psycopg:
	python -V | grep PyPy || $(py_warn) TORTOISE_TEST_DB="psycopg://postgres:$(TORTOISE_POSTGRES_PASS)@$(TORTOISE_POSTGRES_HOST):5432/test_\{\}" pytest $(pytest_opts) --cov-report=

test_mysql_myisam:
	$(py_warn) TORTOISE_TEST_DB="mysql://root:$(TORTOISE_MYSQL_PASS)@$(TORTOISE_MYSQL_HOST):3306/test_\{\}?storage_engine=MYISAM" pytest $(pytest_opts) --cov-report=

test_mysql:
	$(py_warn) TORTOISE_TEST_DB="mysql://root:$(TORTOISE_MYSQL_PASS)@$(TORTOISE_MYSQL_HOST):3306/test_\{\}" pytest $(pytest_opts) --cov-report=
...
test_mssql:
	$(py_warn) TORTOISE_TEST_DB="mssql://sa:$(TORTOISE_MSSQL_PASS)@$(TORTOISE_MSSQL_HOST):1433/test_\{\}?driver=$(TORTOISE_MSSQL_DRIVER)&TrustServerCertificate=YES" pytest $(pytest_opts) --cov-report=
